...
| Автоматические тесты | Проверено | 36/36 тестов проходят; остаются 4 предупреждения о deprecated API |
...
|---|---|---|
| `annotated/annotated_input.*` | Кадр с рамками, видом, фазой и флагом `(!)` | Визуальная проверка |
...
Baseline WeedBlaster является default, потому что он воспроизводимо прошёл полевой demo-run. Class ID не зашит в код: pipeline находит `Weed` по имени.
...
Это демонстрация работоспособности, но не precision/recall и не доказательство распространённости видов: пять кадров не имеют object-level ground truth.
...
README находится в `case1`, но runtime ожидает `case1_main.py` и baseline checkpoint уровнем выше. Если публикуется только содержимое `case1/`, проект пока не самостоятельный. Нужно либо публиковать runtime root целиком, либо перенести entrypoint/weights внутрь standalone-пакета, исправить пути и проверить clean clone.
