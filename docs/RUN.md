...
Полный набор зелёный «из коробки» после `git lfs pull` (`62 passed, 1 skipped` на момент написания этого раздела; после geo/HITL/perennial-модулей тестов стало больше — например, со всеми локальными артефактами на месте `pytest case1/tests -q` даёт `125 passed, 3 skipped`, см. `docs/CHANGELOG_POST_HACKATHON.md`). Итоговое число зависит от того, сколько тестовых файлов накопилось к моменту прогона и что подтянуто локально — ориентируйтесь на собственный вывод команды ниже, а не на число в этом абзаце.
